    ...
    public class GaussianFileReader
    {
        ...
        public static unsafe void ReadFile(string filePath, out NativeArray<InputSplatData> splats)
        {
            if (isPLY(filePath))
            {
                ...
                var vidAttr = attributes.Find(a => a.Item1 == "vertex_id");
                Debug.Log($"vertex_id type = {vidAttr.Item2}");

                int shCoeffCount;
                string attrError = CheckPLYAttributes(attributes, out shCoeffCount);
                if (!string.IsNullOrEmpty(attrError))
                {
                    plyRawData.Dispose();
                    throw new IOException($"PLY file is probably not a Gaussian Splat file? {attrError}");
                }
                ...
                list.Sort((a, b) => a.vertex_id.CompareTo(b.vertex_id));
                ...
                for (int i = 0; i < splats.Length; i++)
                {
                    if (splats[i].vertex_id != i)
                        Debug.LogError($"Mismatch: {i} vs {splats[i].vertex_id}");
                }
                //
        ...
        */
        ...
        //HJ
        static string CheckPLYAttributes(List<(string, PLYFileReader.ElementType)> attributes, out int shCoeffCount)
        {
            shCoeffCount = 0;

            string[] requiredFloat =
            {
                "x", "y", "z",
            ...
            List<string> missing = requiredFloat
                .Where(req => !attributes.Contains((req, PLYFileReader.ElementType.Float)))
                .ToList();

            if (!attributes.Any(a => a.Item1 == "vertex_id"))
                missing.Add("vertex_id");

            if (missing.Count > 0)
                return $"Missing required properties: {string.Join(", ", missing)}";

            List<int> fRestIndices = new List<int>();
            ...
            foreach (var attr in attributes)
            {
                ...
                if (!attr.Item1.StartsWith("f_rest_"))
                    continue;

                string tail = attr.Item1.Substring("f_rest_".Length);
                if (!int.TryParse(tail, out int idx))
                    return $"Invalid f_rest property name: {attr.Item1}";

                fRestIndices.Add(idx);
            }

            fRestIndices.Sort();

            int fRestFloatCount = fRestIndices.Count;

            if (fRestFloatCount != 24 && fRestFloatCount != 45)
                return $"Unsupported f_rest count: {fRestFloatCount}. Expected 24 (SH2) or 45 (SH3).";

            for (int i = 0; i < fRestIndices.Count; i++)
            {
                if (fRestIndices[i] != i)
                    return $"f_rest indices must be contiguous from 0. Expected f_rest_{i}, got f_rest_{fRestIndices[i]}";
            }

            shCoeffCount = fRestFloatCount / 3;
            return null;
        }
        /*
        ...
        */
            ...
            List<(string, PLYFileReader.ElementType)> attributes)
        {
            ...
            string[] splatAttributes =
            {
                ...
                "opacity",
                "scale_0", "scale_1", "scale_2",
                "rot_0", "rot_1", "rot_2", "rot_3","vertex_id",
            };
            ...
            NativeArray<InputSplatData> dst =
                new NativeArray<InputSplatData>(count, Allocator.Persistent, NativeArrayOptions.ClearMemory);

            ReorderPLYData(
                count,
                (byte*)input.GetUnsafeReadOnlyPtr(),
                stride,
                (byte*)dst.GetUnsafePtr(),
                UnsafeUtility.SizeOf<InputSplatData>(),
        ...
        */
        ...
        static unsafe void ReorderSHs(int splatCount, float* data, int shCount)
        {
            ...
            const int maxShCount = 15;

            int splatStride = UnsafeUtility.SizeOf<InputSplatData>() / 4;
            int shStartOffset = 9; // xyz,normals,dc0 다음부터 f_rest 시작

            float* tmp = stackalloc float[maxShCount * 3];
            int idx = shStartOffset;

            for (int i = 0; i < splatCount; ++i)
            {
                // 입력: planar [R1..Rk, G1..Gk, B1..Bk]
                // 출력: interleaved [sh1.r, sh1.g, sh1.b, sh2.r, sh2.g, sh2.b, ...]
                ...
                // 남는 슬롯은 0 padding
                for (int j = shCount * 3; j < maxShCount * 3; ++j)
                    data[idx + j] = 0.0f;
